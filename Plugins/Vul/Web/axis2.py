...
class Detect(threading.Thread):
    ...
    def check(self, url):
        axis2_url = url + '/axis2/services/Cat/getClassPath'
        try:
            res = requests.get(url=axis2_url, headers=self.headers, proxies=self.proxies, timeout=20, verify=False, allow_redirects=False)
            # print(the_md5)
            if "http://ws.apache.org/axis2" in res.text:
                tqdm.write(Fore.RED + '[{}] {}'.format('Axis2', url))
                self.vul_list.append(['Axis2', url, 'Maybe Any file is written'])
                return True
            else:
                return False
