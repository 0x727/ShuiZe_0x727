...
class Detect(threading.Thread):
    ...
    def run_detect(self, url):
        try:
            res = requests.get(url=url, headers=self.headers, proxies=self.proxies, timeout=10)
            if 'realor' in res.text:
                tqdm.write(Fore.RED + '[Realor] {}'.format(url))
                self.vul_list.append(['Realor', url, 'Maybe'])
            else:
                pass
