...
def run_observerWard(alive_Web):
    ...
    # 赋予执行权限
    os.system('chmod 777 {}/observerWard'.format(observerWardFolder))

    # 更新observerWard的指纹库
    observerWardUpdateCMD = '{}/observerWard --update_fingerprint'
    ...
    with open(observerWardResultPath, 'rt', encoding='utf-8') as f:
        ...
        if text:
            for each in json.loads(text):
                ...
                if vulName:
                    # print(url, vulName)
                    observerWardVul_list.append([vulName, url])
