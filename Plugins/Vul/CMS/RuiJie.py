...
class Detect(threading.Thread):
    ...
    # 锐捷易网关RCE
    def webVul1(self):
        url1 = self.url + '/guest_auth/guestIsUp.php'
        tqdm.write(Fore.WHITE + 'test RCE : {}'.format(url1))
        try:
            res = requests.get(url=url1, headers=self.headers, proxies=self.proxies, verify=False, allow_redirects=False)
            if res.status_code == 200:
                self.isExist = True
                tqdm.write(Fore.RED + '[锐捷易网关RCE] {}'.format(url1))
                self.vul_list.append(['锐捷易网关RCE', self.url, 'Yes {}'.format(url1)])
